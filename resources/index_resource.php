...
class index_resource extends app_resource{
	...
	function GET($offset = 0, $limit = 5){
		...
		*/
		...
		if($this->most_recent_post !== null)  $this->posts = storage::find_posts(array("where"=>"ROWID != :id and owner_id=:owner_id and status='public'", "args"=>array("owner_id"=>site::$member->id, "id"=>$this->most_recent_post->id), "order_by"=>"publish_date desc", "offset"=>$this->offset, "limit"=>$this->limit));
